...
from aiogram.utils.i18n import lazy_gettext as lazytext
...
async def check_and_load_id(message: Message, state: FSMContext):
    if await aqt_db.check_thread_id(message.text):
        ...
        await message.reply(gettext("Okay, write a question ^-^"))
        await state.set_state(ThreadAsker.question)
    else:
        await message.reply(gettext("This code doesn't exist in DB. Type valid code"))
...
async def load_question(message: Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=user_id[0],
                           text=gettext("You got a question from {data} thread\n\n{message}"
                                        "\n\nClick on ✅ to answer on question.\n"
                                        "Click on ❌ if u don't want to answer, but need give "
    ...
    await message.reply(gettext("Okay, I sent your message to {data} thread".format(data=data['thread_id'])),
...
async def load_answer(message: Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=user_id[0],
                           text=gettext("You got a question from {code} thread\n\n{message}"
                                        .format(code=code[0], message=message.text)),
                           parse_mode="Markdown",
                           reply_markup=get_start_kb())
    await message.reply(text=gettext("Okay, I sent your answer"),
                        reply_markup=get_start_kb())
    await state.clear()
...
async def log_accept_handler(callback: CallbackQuery):
    ...
    if callback.data == "accept":
        await callback.message.answer(text=gettext("Do you want to create a new question "
                                                   "thread's link or write a question to "
                                                   "the already exist thread?"),
                                      reply_markup=get_start_kb())
    elif callback.data == "cancel":
        await callback.message.answer(text=gettext("Okay, but you can't use this bot without "
...
async def get_answer_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data == "answer":
        await callback.message.answer(text=gettext("Okay, write an answer: "))
        await state.set_state(ThreadOwner.answer)
    elif callback.data == "skip":
        await bot.send_message(chat_id=chat_id[0],
                               text=gettext("Your answer has been watched by thread owner, "
                                            "but he preferred to ignore it."))
        await callback.message.answer(gettext("Okay, I sent your feedback"))
